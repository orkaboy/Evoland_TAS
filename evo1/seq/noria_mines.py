    ...
    def __init__(self):
        super().__init__(
            ...
            children=[
                ...
                SeqMove2DClunkyCombat("Maze", coords=_noria_astar.calculate(start=Vec2(27, 24), goal=Vec2(15, 23))),
                # TODO: Open door(N)
                SeqMove2D("Door", coords=[Vec2(15, 22)]),
                SeqMove2DClunkyCombat("Juke skellies", coords=_noria_astar.calculate(start=Vec2(15, 21), goal=Vec2(14, 18))),
                SeqMove2DClunkyCombat("Juke skellies", coords=_noria_astar.calculate(start=Vec2(14, 18), goal=Vec2(9, 14))),
                # Get in position for chest
                SeqMove2D("Move to chest", coords=[Vec2(9, 13.6)]),
                SeqGrabChest("Push blocks", direction=Facing.UP),
                SeqMove2DClunkyCombat("Move to block", coords=_noria_astar.calculate(start=Vec2(9, 14), goal=Vec2(12, 15))),
                SeqMove2D("Push block", coords=[Vec2(12, 13)]),
                SeqMove2DClunkyCombat("Move to block", coords=_noria_astar.calculate(start=Vec2(12, 15), goal=Vec2(4, 4))),
                SeqMove2D("Push block", coords=[Vec2(4, 3)]),
                SeqMove2DClunkyCombat("Move to block", coords=_noria_astar.calculate(start=Vec2(4, 4), goal=Vec2(11, 4))),
                SeqMove2D("Push block", coords=[Vec2(11, 3)]),
                SeqMove2DClunkyCombat("Move to trap", coords=_noria_astar.calculate(start=Vec2(11, 4), goal=Vec2(22, 15))),
                # TODO: Kill skellies (reuse knight combat) [19|20|21, 12]
                SeqManualUntilClose("KILL SKELLIES", target=Vec2(22, 17)),
                # TODO: Remove manual
                SeqMove2DClunkyCombat("Move to chest", coords=_noria_astar.calculate(start=Vec2(22, 15), goal=Vec2(22, 17))),
                # Get in position for chest
                SeqMove2D("Move to chest", coords=[Vec2(22, 17.4)]),
                SeqGrabChest("GUI", direction=Facing.DOWN),
                SeqMove2DConfirm("Talk", coords=[Vec2(26, 18)]),
                SeqMove2DClunkyCombat("Move to chest", coords=_noria_astar.calculate(start=Vec2(26, 18), goal=Vec2(33, 17))),
                # Get in position for chest
                ...
                # TODO: Remove manual
                SeqMove2DClunkyCombat("Trigger plate(L)", coords=_noria_astar.calculate(start=Vec2(34, 5), goal=Vec2(32, 4))),
                SeqHoldInPlace(name="Trigger plate(L)", target=Vec2(32, 4), timeout_in_s=0.5),
                SeqMove2DClunkyCombat("Trigger plate(R)", coords=_noria_astar.calculate(start=Vec2(32, 4), goal=Vec2(36, 4))),
                SeqHoldInPlace(name="Trigger plate(R)", target=Vec2(36, 4), timeout_in_s=0.5),
                # TODO: Just move here? Ignore skellies?
                SeqMove2DClunkyCombat("Move to chest", coords=_noria_astar.calculate(start=Vec2(36, 4), goal=Vec2(38, 14))),
                # Get in position for chest
                ...
                SeqMove2DClunkyCombat("Move to door", coords=_noria_astar.calculate(start=Vec2(61, 27), goal=Vec2(51, 24))),
                # TODO: Open door(N)
                SeqMove2D("Door", coords=[Vec2(51, 23)]),
                SeqMove2DClunkyCombat("Move to chest", coords=_noria_astar.calculate(start=Vec2(51, 23), goal=Vec2(53, 19))),
                # Get in position for chest
                SeqMove2D("Move to chest", coords=[Vec2(53, 18.6)]),
                SeqGrabChest("Lava", direction=Facing.UP),
                SeqMove2DClunkyCombat("Move to trigger", coords=_noria_astar.calculate(start=Vec2(53, 18), goal=Vec2(66, 20))),
                ...
                # TODO: Remove manual
                SeqMove2DClunkyCombat("Move to chest", coords=_noria_astar.calculate(start=Vec2(70, 51), goal=Vec2(78, 56))),
                # Get in position for chest
                ...
                SeqMove2DClunkyCombat("Move to trigger", coords=_noria_astar.calculate(start=Vec2(41, 53), goal=Vec2(32, 51))),
                SeqMove2DConfirm("Talk", coords=[Vec2(32, 54)]),
                SeqMove2DClunkyCombat("Move to trigger", coords=_noria_astar.calculate(start=Vec2(32, 54), goal=Vec2(27, 62))),
