...
class SeqKnight2D(SeqSection2D):
    class _Plan:
        def __init__(
            self, mem: ZeldaMemory, targets: List[Vec2], track_size: float
        ) -> None:
            ...
            with contextlib.suppress(
                ...
                for actor in mem.actors:
                    if actor.kind != GameEntity2D.EKind.ENEMY:
                        continue
                    enemy_pos = actor.pos
                    search_box = get_box_with_size(
                        center=enemy_pos, half_size=track_size
                    )
            ...
            if len(targets) != len(self.targets):
                logger.error(
                    f"Couldn't track all entities! Found {len(self.targets)}/{len(targets)} enemies"
                )
        ...
        def get_next_target(self) -> Optional[GameEntity2D]:
            ...
            key_list = [
                (dist(player_pos, target.pos), target) for target in self.targets
            ]
    ...
    def execute(self, delta: float) -> bool:
        ...
        if self.plan is None:
            self.plan = SeqKnight2D._Plan(
                mem=mem, targets=self.target_coords, track_size=self.track_size
            )
    ...
    def _try_move_into_position_and_attack(self, target: GameEntity2D) -> bool:
        ...
        for enemy in self.plan.targets:
            ...
            )  # TODO: enemy collision magic number. Get bounds
        ...
        mem = get_zelda_memory()
        player_pos = mem.player.pos
        closest_weak_spot = self._find_closest_point(
            origin=player_pos, points=attack_vectors
        )
