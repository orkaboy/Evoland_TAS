    ...
    class Header(KaitaiStruct):
        ...
        def _read(self):
            ...
            if not self.magic1 == b"\x50\x41\x4B":
                raise kaitaistruct.ValidationNotEqualError(
                    b"\x50\x41\x4B", self.magic1, self._io, "/types/header/seq/0"
                )
            ...
            self.root_entry = HeapsPak.Header.Entry(
                _io__raw_root_entry, self, self._root
            )
            self.magic2 = self._io.read_bytes(4)
            if not self.magic2 == b"\x44\x41\x54\x41":
                raise kaitaistruct.ValidationNotEqualError(
                    b"\x44\x41\x54\x41", self.magic2, self._io, "/types/header/seq/5"
                )
        ...
        class Entry(KaitaiStruct):
            ...
            def _read(self):
                ...
                self.name = (self._io.read_bytes(self.len_name)).decode("UTF-8")
        ...
        class Dir(KaitaiStruct):
            ...
            def _read(self):
                ...
                for i in range(self.num_entries):
                    self.entries.append(
                        HeapsPak.Header.Entry(self._io, self, self._root)
                    )
